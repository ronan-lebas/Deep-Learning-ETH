...
class ImagePreprocessor:
    ...
    def __init__(self, mode: str, split: str, path_to_dataset='.', transform=None):
        self.base_folder = (
            path_to_dataset+f"/YOLO_dataset/"  # Hardcoded base folder for processed data
        )
        self.original_folder = (
            path_to_dataset+f"/dataset/{mode}/"  # Hardcoded folder for raw dataset
        )
        self.mode = mode
        self.split = "train" if split == "train" else "val"
        self.images_folder = os.path.join(self.base_folder, self.mode, self.split, "images")
        self.labels_folder = os.path.join(self.base_folder, self.mode, self.split, "labels")
